# SCRUM : Daily scrum du mardi matin 06 janvier 2026 | Scrum Master: Axelle

ATTENTION : c'est **le lecteur qui doit comprendre** ce qui a été fait et par qui. Donnez donc suffisamment de substance ici pour cela. Rien de plus.

## Ce qui a été fait la semaine passée

### AXELLE
- Avancement de la documentation
- Création du web summary
- Création du PowerPoint
- Avancement des tests

### Noe
- Aide à la mise en place de la Raspberry sur le grand écran
- Amélioration des jeux et de l’arcade
- Amélioration du jeu 3




### Diogo
- Installation et configuration de la Raspberry
- Création d’une documentation d’installation


### Valentin
- Documentation
- Gestion des tâches
- Mise en place de la Raspberry sur le grand écran
- Planification du sprint



## Ce qui sera fait aujourd'hui

### AXELLE
- Finir les tests
- Finaliser la documentation
- Finaliser la présentation



### Noe
- Finaliser la sécurité et corriger quelques bugs
- Améliorer la police


### Diogo
- Avancement de la documentation et de la documentation de projet


### Valentin
- Finaliser la documentation
- Réfléchir à de nouvelles fonctionnalités
- Tester et vérifier que toutes les fonctionnalités du cahier des charges sont présentes




## Problèmes rencontrés

-   Aucun problème majeur à ce jour.

# Instructions

Ce document est complété durant la cérémonie du `Daily Scrum`.

1. Tenir également à jour le Kanban.
