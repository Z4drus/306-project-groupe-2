# SCRUM : Daily scrum du mardi matin 12 janvier 2026 | Scrum Master: Axelle

ATTENTION : c'est **le lecteur qui doit comprendre** ce qui a été fait et par qui. Donnez donc suffisamment de substance ici pour cela. Rien de plus.

## Ce qui a été fait la semaine passée

### AXELLE
- Réalisation des test 
- Rédaction de la documentation

### Noe
- Amelioration du code
- Correction des bugs



### Diogo
- Rédaction d'installation
- Création du script d'installtion 


### Valentin
- Rédaction de la documentation
- Discussion de nouvelles fonctionnalités
- validation des tâches et fonctionnalités

## Ce qui sera fait aujourd'hui

### AXELLE
- préparer la présentation
- finir le fichier Scrum dans l’ordre


### Noe
- mettre en place les fonctionnalités supplémentaires
- préparer la présentation
- documentation du projet

### Diogo
- finir la documentation d’installation Raspberry

### Valentin
- tester l’application
- valider les dernières fonctionnalités
- préparer la présentation



## Problèmes rencontrés

-   Aucun problème majeur à ce jour.

# Instructions

Ce document est complété durant la cérémonie du `Daily Scrum`.

1. Tenir également à jour le Kanban.
